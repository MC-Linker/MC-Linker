...
async function execute(message, args) {
    ...
    if(!message.member.permissions.has(Discord.PermissionFlagsBits.KickMembers)) {
        message.replyTl(keys.commands.kick.warnings.no_permission);
        return;
    }
    else if(!user) {
        message.replyTl(keys.commands.kick.warnings.no_username);
        return;
    }
    ...
    if(resp.status === 206) message.replyTl(keys.commands.kick.warnings.response_warning, { username: user, reason });
    else message.replyTl(keys.commands.kick.success, { username: user, reason });
}
